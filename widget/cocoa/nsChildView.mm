...
// Compatibility defines for 10.4 SDK
#if !defined(MAC_OS_X_VERSION_10_5) || (MAC_OS_X_VERSION_MAX_ALLOWED < MAC_OS_X_VERSION_10_5)
// kCGBlendModeCopy was introduced in 10.5; it's the 18th value in the enum.
#ifndef kCGBlendModeCopy
#define kCGBlendModeCopy ((CGBlendMode)17)
#endif
// NSRectToCGRect was introduced in 10.5; on 32-bit the types are layout-compatible.
...
UnderlineAttributeToTextRangeType(PRUint32 aUnderlineStyle, NSRange selRange)
{
  ...
  if (selRange.length == 0) {
    switch (aUnderlineStyle) {
      case 1:
        attr = TextRangeType::eRawClause;
        break;
      case 2:
      default:
        attr = TextRangeType::eSelectedRawClause;
        break;
    }
  }
  else {
    switch (aUnderlineStyle) {
      case 1:
        attr = TextRangeType::eConvertedClause;
        break;
      case 2:
      default:
        attr = TextRangeType::eSelectedClause;
        break;
    }
  }
  return attr;
...
template<typename F>
static void
ManipulateViewWithoutNeedingDisplay(NSView* aView, F aCallback)
{
  BaseWindow* win = nil;
...
nsChildView::AttachNativeKeyEvent(mozilla::WidgetKeyboardEvent& aEvent)
{
  ...
  if (aEvent.mNativeKeyEvent || aEvent.mModifiers & MODIFIER_OS) {
    return NS_OK;
  }
  ...
  for (uint32_t i = 0; i < ArrayLength(sModifierFlagMap); ++i) {
    if (aEvent.mModifiers & sModifierFlagMap[i][0]) {
      modifierFlags |= sModifierFlagMap[i][1];
    }
  }
  ...
  if (aEvent.mCharCode) {
    characters = [NSString stringWithCharacters:
      reinterpret_cast<const unichar*>(&(aEvent.mCharCode)) length:1];
  } else {
    uint32_t cocoaCharCode =
      nsCocoaUtils::ConvertGeckoKeyCodeToMacCharCode(aEvent.mKeyCode);
    characters = [NSString stringWithCharacters:
...
static void
AppendRoundedRectToPath(NSBezierPath* aPath, NSRect aPathRect, CGFloat aInnerRadius)
{
#if defined(MAC_OS_X_VERSION_10_5) && MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_5
  [aPath appendBezierPathWithRoundedRect:aPathRect xRadius:aInnerRadius yRadius:aInnerRadius];
#else
  float clampedRadius = MIN(aInnerRadius, 0.5 * MIN(aPathRect.size.width, aPathRect.size.height));
  NSPoint topLeft = NSMakePoint(NSMinX(aPathRect), NSMaxY(aPathRect));
  NSPoint topRight = NSMakePoint(NSMaxX(aPathRect), NSMaxY(aPathRect));
  NSPoint bottomRight = NSMakePoint(NSMaxX(aPathRect), NSMinY(aPathRect));
  [aPath moveToPoint:NSMakePoint(NSMidX(aPathRect), NSMaxY(aPathRect))];
  [aPath appendBezierPathWithArcFromPoint:topLeft
        toPoint:aPathRect.origin radius:clampedRadius];
  [aPath appendBezierPathWithArcFromPoint:aPathRect.origin
  ...
  [aPath appendBezierPathWithArcFromPoint:topRight toPoint:topLeft
...
DrawTitlebarHighlight(NSSize aWindowSize, CGFloat aRadius, CGFloat aDevicePixelWidth)
{
  ...
  BOOL drawGradient = YES;
#if defined(MAC_OS_X_VERSION_10_5) && MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_5
  drawGradient = nsCocoaFeatures::OnLeopardOrLater();
#else
  drawGradient = NO;
#endif
  ...
  CGFloat clipInset = drawGradient ? aDevicePixelWidth : 2 * aDevicePixelWidth;
  NSRect innerRect = NSInsetRect(pathRect, clipInset, clipInset);
  CGFloat innerRadius = aRadius - clipInset;
  AppendRoundedRectToPath(path, innerRect, innerRadius);
  [path addClip];
  ...
  if (drawGradient) {
    ...
    // Tiger has a different looking highlight, approximate
    [NSGraphicsContext saveGraphicsState];
    NSBezierPath* bottomClip = [NSBezierPath bezierPathWithRect:NSMakeRect(0, 0, aWindowSize.width, aRadius / 2)];
    [bottomClip addClip];

    [[NSColor colorWithDeviceWhite:1.0 alpha:120.0/255.0] set];
    NSRectFillUsingOperation(NSMakeRect(0, 0, aWindowSize.width, aRadius + 2), NSCompositeSourceOver);

    NSBezierPath* outerPath = [NSBezierPath bezierPath];
    [outerPath setWindingRule:NSEvenOddWindingRule];
    [outerPath appendBezierPathWithRect:pathRect];
    NSRect outerInnerRect = NSInsetRect(pathRect, aDevicePixelWidth, aDevicePixelWidth);
    CGFloat outerInnerRadius = aRadius - aDevicePixelWidth + 1;
    AppendRoundedRectToPath(outerPath, outerInnerRect, outerInnerRadius);
    [outerPath addClip];

    [[NSColor colorWithDeviceWhite:1.0 alpha:253.0/255.0] set];
    NSRectFillUsingOperation(NSMakeRect(0, 0, aWindowSize.width, aRadius + 2), NSCompositeSourceOver);
...
- (void) convertCocoaKeyEvent:(NSEvent*)aKeyEvent toGeckoEvent:(WidgetKeyboardEvent*)outGeckoEvent
{
  ...
  if (outGeckoEvent->mMessage == eKeyPress &&
    ...
    outGeckoEvent->mIsChar = true; // this is not a special key

    outGeckoEvent->mCharCode = 0;
    outGeckoEvent->mKeyCode  = 0; // not set for key press events
    outGeckoEvent->mLocation = ConvertMacToGeckoKeyLocation(keyCode);

    NSString* chars = [aKeyEvent characters];
    if ([chars length] > 0)
      outGeckoEvent->mCharCode = [chars characterAtIndex:0];

    // convert control-modified charCode to raw charCode (with appropriate case)
    if (outGeckoEvent->IsControl() && outGeckoEvent->mCharCode <= 26)
      outGeckoEvent->mCharCode += (outGeckoEvent->IsShift()) ? ('A' - 1) : ('a' - 1);
    ...
    if (outGeckoEvent->IsControl() || outGeckoEvent->IsMeta() ||
      ...
      if ((unshiftedChar || shiftedChar) &&
        ...
        outGeckoEvent->mAlternativeCharCodes.AppendElement(altCharCodes);
      }
...
#ifdef DEBUG_KB
          if (outGeckoEvent->mCharCode != preferredCharCode) {
            NSLog(@"      charCode replaced: %X(%C) to %X(%C)",
                  outGeckoEvent->mCharCode,
                  outGeckoEvent->mCharCode > ' ' ? outGeckoEvent->mCharCode : ' ',
                  preferredCharCode,
                  preferredCharCode > ' ' ? preferredCharCode : ' ');
          }
#endif
          outGeckoEvent->mCharCode = preferredCharCode;
        }
      }
    ...
    outGeckoEvent->mKeyValue = (uint32_t)outGeckoEvent->mCharCode;
  }
  else {
    ...
    outGeckoEvent->mKeyCode = ConvertMacToGeckoKeyCode(keyCode, characters);
    outGeckoEvent->mCharCode = 0;
...
- (void)sendCompositionEvent:(EventMessage) aEventType
{
#ifdef DEBUG_IME
                      ...
                      doCommit:(BOOL) doCommit
{
  ...
  textEvent.mTime = PR_IntervalNow();
...
#define MAX_BUFFER_SIZE 32
  ...
  [tmpStr getCharacters:reinterpret_cast<unichar*>(bufPtr)];
  ...
  if (len == 1 && !nsTSMManager::IsComposing()) {
    ...
    geckoEvent.mCharCode  = bufPtr[0]; // gecko expects OS-translated unicode
    geckoEvent.mKeyCode   = 0;
    geckoEvent.mIsChar    = true;
    ...
    if (mCurKeyEvent) {
      ...
      uint32_t keyCode = [mCurKeyEvent keyCode];
      ...
      if (!IsPrintableChar(geckoEvent.mCharCode)) {
        geckoEvent.mKeyCode = ConvertMacToGeckoKeyCode(keyCode, [mCurKeyEvent charactersIgnoringModifiers]);
        geckoEvent.mCharCode = 0;
        geckoEvent.mKeyValue = (uint32_t)geckoEvent.mKeyCode;
      }
      geckoEvent.mCodeNameIndex = ConvertMacToDOMCode(keyCode);
      geckoEvent.mLocation = ConvertMacToGeckoKeyLocation(keyCode);
    } else {
      ...
      if (!IsPrintableChar(geckoEvent.mCharCode)) {
        geckoEvent.mKeyCode = GetGeckoKeyCodeFromChar(geckoEvent.mCharCode);
        geckoEvent.mCharCode = 0;
        geckoEvent.mKeyValue = (uint32_t)geckoEvent.mKeyCode;
      }
      ...
      geckoEvent.mLocation       = nsIDOMKeyEvent::DOM_KEY_LOCATION_STANDARD;
    }

    // Remove basic modifiers from keypress event because if they are included,
    // nsPlaintextEditor ignores the event.
    geckoEvent.mModifiers &= ~(mozilla::MODIFIER_CONTROL |
...
- (NSRect) firstRectForCharacterRange:(NSRange)theRange
{
  ...
  if (!rootWindow || !rootView)
    return rect;
  rect = nsCocoaUtils::DevPixelsToCocoaPoints(r, 1.0);
...
((nsTSMManager::GetIMEOpenState()) ? "TRUE" : "false")));
  ...
  if (nonDeadKeyPress && !nsTSMManager::IsComposing()) {
    ...
    if ((!geckoKeypress.mIsChar || geckoKeypress.IsControl()) &&
...
- (void)fireKeyEventForFlagsChanged:(NSEvent*)theEvent keyDown:(BOOL)isKeyDown
{
  ...
  EventMessage message = isKeyDown ? eKeyDown : eKeyUp;
